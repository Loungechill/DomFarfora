...
DEFAULT_SOURCE = "https://api.domfarfora.ru/bitrix/catalog_export/yandex-direct.xml"
...
def download_source(source: str, destination: Path) -> None:
    if source.startswith(("http://", "https://")):
        request = urllib.request.Request(
            source,
            headers={
                "User-Agent": "DomFarfora-Feed-Filter/1.0 (+GitHub Actions)",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Filter Dom Farfora YML/XML feed by <vendor> blacklist")
    ...
    with tempfile.TemporaryDirectory(prefix="dom-farfora-feed-") as tmpdir:
